...
type Weather = { temperature: number; code: number; isDay: boolean; location?: string }
...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchWeather = async (latitude: number, longitude: number, location?: string) => {
      try {
        ...
        url.searchParams.set('current', 'temperature_2m,weather_code,is_day'); url.searchParams.set('timezone', 'auto')
        ...
        if (payload.current?.temperature_2m === undefined || payload.current.weather_code === undefined || payload.current.is_day === undefined) throw new Error('weather')
        if (!disposed) setWeather({ temperature: payload.current.temperature_2m, code: payload.current.weather_code, isDay: payload.current.is_day === 1, location })
  ...
  const storageDetail = status?.storage_source === 'pbs' && status.pbs_error ? 'PBS nicht erreichbar' : `${bytes(cluster.storage_used)} von ${bytes(cluster.storage_total)} belegt`
  ...
  return <div className={`dashboard ${updateLocked ? 'update-locked' : ''}`}>
    ...
    <header className="topbar">
      ...
      <button className="welcome" type="button" onClick={() => setWeatherAttempt(value => value + 1)} title="Standortwetter erneut laden"><Icon name={weather ? weatherIcon(weather.code, weather.isDay) : hour >= 18 || hour < 6 ? 'cloud-moon' : 'cloud-sun'} size={38} /><p><strong>{greeting}</strong><span>{weather ? `${Math.round(weather.temperature)} °C · ${weatherText(weather.code)}` : weatherMessage}</span></p></button>
    ...
    <main>
      <section className="status-grid">
        ...
        <StatusCard icon="database" title="Speichernutzung" value={`${storage}%`} detail={storageDetail} progress={storage} />
        <StatusCard icon="cloud" title="Letztes Backup" value={backupLabel} detail={status?.pbs_error && status.pbs_configured ? 'PBS nicht erreichbar' : status?.latest_backup ? 'Sicherungsstatus erfasst' : 'Wartet auf Status'} accent />
      </section>
